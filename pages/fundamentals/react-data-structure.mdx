...
In this part we will have a glance through the core component affects the data structure in React, including `FiberRoot`, `Fiber`, `effectTags`, `ReactWorkTag`, `Update` and `UpdateQueue` and compare both the latest version and the version on v16. More details will be discussed in later section. 
